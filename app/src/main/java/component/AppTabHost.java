 ...
 */
public class AppTabHost extends TabHost implements
	...
	private Map mTabMap = new HashMap();
	...
	public AppTabHost(Context context) {
	...
	public AppTabHost(Context context, AttributeSet attrs) {
		super(context, attrs);
...
*/
	...
	public boolean keepState(String tag) {
		if (tag == null) {
			return false;
		}
		Object o = mTabMap.get(tag);
		if (o == null) {
			return false;
		}
		return Boolean.valueOf(mTabMap.get(tag).toString());
	}
	
	/*public void addTab(TabSpec tabSpec, Class<?> clss, Bundle args) {
		addTab(tabSpec, clss, false, args);
	}*/
	
	public void addTab(TabSpec tabSpec, Class<?> clss, boolean keepState, Bundle args) {
		tabSpec.setContent(new DummyTabFactory(mContext));
		String tag = tabSpec.getTag();
		mTabMap.put(tag, keepState);
		...
		if (mLastTab != newTab) {
			...
			if (newTab != null) {
				if (newTab.fragment == null) {
					...
					Fragment fragment = newTab.fragment;
					if (keepState(fragment.getTag())) {
						ft.show(fragment);
					} else {
						ft.attach(fragment);
